...
//Java implementation of an O(n) approach of level order traversal in spiral form 

import java.util.*;

public class SpiralLevelOrderUsing2Stack_IMP_3 {
	...
	void printSpiral(Node node) {
		...
		while (!s1.empty() || !s2.empty()) {
			...
			while (!s1.empty()) {
				...
				// IMPORTANT: right is pushed before left
				if (temp.right != null)
					s2.push(temp.right);
			...
			while (!s2.empty()) {
				...
				Node temp = s2.pop();
				System.out.print(temp.data + " ");
				// IMPORTANT left is pushed before right
				if (temp.left != null)
					s1.push(temp.left);
				if (temp.right != null)
					s1.push(temp.right);
	...
	public static void main(String[] args) {
		SpiralLevelOrderUsing2Stack_IMP_3 tree = new SpiralLevelOrderUsing2Stack_IMP_3();
