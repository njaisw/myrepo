...
public class ConvertPreOrder2PostOrder_IMP_3 {
	static class INT {
		int data;
...
//Function to find postorder traversal from preorder traversal. 
	static void findPostOrderUtil(int pre[], int n, int minval, int maxval, INT preIndex) {
		...
		// If entire preorder array is traversed then return as no more element is left
		// to be added to post order array.
		if (preIndex.data == n)
			return;

		// If array element does not lie in range specified, then it is not part of
		// current subtree.
		if (pre[preIndex.data] < minval || pre[preIndex.data] > maxval) {
			return;
		}

		// Store current value, to be printed later, after printing left and right
		// subtrees. Increment preIndex to find left and right subtrees, and pass this
	...
	static void findPostOrder(int pre[], int n) {
		// To store index of element to be traversed next in preorder array. This is
		// passed by reference to utility function.
