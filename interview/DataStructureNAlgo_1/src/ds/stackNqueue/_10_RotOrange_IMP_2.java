...
//Java program to find minimum time required to make all oranges rotten 

import java.util.LinkedList;
import java.util.Queue;

public class _10_RotOrange_IMP_2 {
	...
	static int rotOranges(int arr[][]) {
		...
		while (!Q.isEmpty()) {
			// determine whether even a single fresh orange gets rotten due to rotten
			// oranges in current time frame so we can increase the count of required time.
			boolean flag = false;
			...
			// If oranges were rotten in current frame than separate the rotten oranges
			// using delimiter for the next frame for processing.
			// Namrata: Whenever the delimiter is removed if queue is not empty then put the
			// delimiter back this would help in count of flag
