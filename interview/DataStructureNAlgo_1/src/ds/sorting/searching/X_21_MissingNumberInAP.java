...
//A Java program to find the missing number in a given arithmetic progression 
...
public class X_21_MissingNumberInAP {
	...
	static int findMissingUtil(int arr[], int low, int high, int diff) {
		...
		// The element just after the middle element is missing. The arr[mid+1] must
		// exist, because we return when (low == high) and take floor of (high-low)/2
	...
	// The function uses findMissingUtil() to find the missing element in AP. It
	// assumes that there is exactly one missing element and may give incorrect
	...
	static int findMissing(int arr[], int n) {
		// If exactly one element is missing, then we can find difference of arithmetic
		// progression using following formula. Example, 2, 4, 6, 10, diff =(10-2)/4= 2.
