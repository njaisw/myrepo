...
public class X_SearchUsingSuffixTree { 

	final static int MAX_CHAR = 256; 

	X_SearchUsingSuffixTree[] children = new X_SearchUsingSuffixTree[MAX_CHAR]; 
	List<Integer> indexes; 

	X_SearchUsingSuffixTree() // Constructor 
	{ 
	...
	void insertSuffix(String s, int index) { 
		...
		if (s.length() > 0) { 
			...
			if (children[cIndex] == null) 
				children[cIndex] = new X_SearchUsingSuffixTree(); 
...
class Suffix_tree{ 
	...
	X_SearchUsingSuffixTree root = new X_SearchUsingSuffixTree(); 
